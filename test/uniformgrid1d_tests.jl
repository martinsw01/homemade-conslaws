...
@testset "Test grid periodic BC" begin
    ...
    for_each_cell(grid, 1) do cells, dx, center_idx1, (left_idx, center_idx2, right_idx)
        @test mod1(left_idx + 1, N+1) == center_idx1 == center_idx2 == mod1(right_idx - 1, N+1)
        @test cells[center_idx1] ≈ expected_cells[center_idx1]
        @test dx ≈ expected_dx
    end

    for_each_cell(grid, 2) do cells, dx, center_idx1, (left_idx, _, center_idx2, _, right_idx)
...
@testset "Test grid neumann BC" begin
    ...
    for_each_cell(grid, 2) do cells, dx, cell_idx, idxs
        @test idxs == expected_neighbour_indices[cell_idx]
    end
end
